...
/* harmony import */ var _content_showBalance__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./content/showBalance */ "./Fiverr-Seller-Assistant crome extension/assets/src/js/content/showBalance.js");
/* harmony import */ var _content_showBalance__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_content_showBalance__WEBPACK_IMPORTED_MODULE_2__);
...
/* harmony import */ var _content_profilePopup__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./content/profilePopup */ "./Fiverr-Seller-Assistant crome extension/assets/src/js/content/profilePopup.js");
/* harmony import */ var _content_profilePopup__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_content_profilePopup__WEBPACK_IMPORTED_MODULE_4__);
/**
 ...
 */
 ...
 // import './content/offerMoneyCalculator';
                ...
                switch (_context.prev = _context.next) {
                  case 0:
                    ...
                    targets.forEach(function (offer) {
                      ...
                      nameElement.classList.add('extension-requestpage-username'); // nameElement.setAttribute('style', 'color: #1dbf73;font-weight: 600;font-size: large; display: block;')
                          ...
                          while (1) {
                            switch (_context4.prev = _context4.next) {
                              case 0:
                                ...
                                fetch("https://api.futurewordpress.com/v1/licenses/fiverr.chrome/" + licenseKey + "/verify/json/", {
                                  method: "POST",
                                  headers: {
                              ...
                              fetch("https://api.futurewordpress.com/v1/licenses/fiverr.chrome/" + licenseKey + "/verify/json/", {
                                ...
                                if (res.status === 200) {
                                  res.json().then(function (json) {
                                    if (json.status) {
                                      thisClass.init();
                                      thisClass.calculator();
                                      setInterval(function () {
                                        thisClass.calculateOffers();
                                      }, 1000);
                                    }
                                  });
                                }
    ...
    _createClass(inboxPopupCalculator, [{
      ...
      value: function init() {
        ...
        var calculateFromSeller = function calculateFromSeller(sellerAmount) {
          var price = sellerAmount / .8; // const price = thisClass.calculate( sellerAmount, 'yg' )

          var buyerAmount = price + (price > 40.0 ? price * .05 : 2); // const buyerAmount = thisClass.calculate( sellerAmount, 'bp' )

          return [price, buyerAmount];
        };

        var calculateFromOrder = function calculateFromOrder(price) {
          var sellerAmount = price * .8; // const sellerAmount = thisClass.calculate( price, 'yg' )

          var buyerAmount = price + (price > 40.0 ? price * .05 : 2); // const buyerAmount = thisClass.calculate( price, 'bp' )
        ...
        div.classList.add('custom-offer-package-milestones', 'custom_offer-package');
        var div1 = document.createElement('div');
        div1.classList.add('milestones-modal-header', 'flex-items-center');
        var h5 = document.createElement('h5');
        h5.innerText = 'Calculate Order Comissions';
        div1.appendChild(h5);
        var button = document.createElement('button');
        button.classList.add('btn-close');
        var span = document.createElement('span');
        span.classList.add('lFICM06');
        thisClass.setAttrs(span, {
          'aria-hidden': 'true',
          'style': 'width: 14px; height: 14px;',
          'onclick': "document.getElementById( 'money-calculator' ).classList.remove( 'visible' )"
        });
        span.innerHTML = thisClass.closeSVG();
        div1.appendChild(span);
        div.appendChild(div1);
        var nav = document.createElement('nav');
        nav.classList.add('milestones-select-gig', 'p-b-20');
        var divs = [document.createElement('fieldset'), document.createElement('fieldset'), document.createElement('fieldset')];
        var inputs = [document.createElement('input'), document.createElement('input'), document.createElement('input')];
        var ps = [document.createElement('legend'), document.createElement('legend'), document.createElement('legend')];

        for (var i = 0; i < 3; ++i) {
          divs[i].id = 'money-calculator-' + i.toString();
          ps[i].textContent = i === 0 ? 'You\'ll Earn' : i === 1 ? 'Estimated Price' : 'Buyer Pays';
          inputs[i].type = 'number';
          divs[i].appendChild(ps[i]);
          divs[i].appendChild(inputs[i]);
          nav.appendChild(divs[i]);
        }

        parent.removeChild(footer);
        div.appendChild(nav);
        moneyCalculatorDiv.appendChild(div);
        ...
        textarea.addEventListener('keyup', function (event) {
          ...
          if (matches !== null) {
            ...
            moneyCalculatorDiv.classList.add('visible');
            ...
            moneyCalculatorDiv.classList.remove('visible');
          }
        });
      ...
      value: function calculator() {
        ...
        span.setAttribute('data-hint', 'Money Calculator (Alt + C)');
        span.onclick = thisClass.clickEventPanel(span);
        var button = document.createElement('button');
        button.type = 'button';
        button.setAttribute('data-testid', 'calculator-button');
        span.appendChild(button);
        span.innerHTML = thisClass.calcSVG();
        elm.appendChild(span);
        document.querySelector('#conversation .message-flow .message-action-bar > ul').appendChild(elm);
        document.addEventListener('keydown', function (e) {
          if (e.key === "Escape") {
            document.getElementById('money-calculator').classList.remove('visible');
          } else if (e.altKey && e.keyCode == 67) {
            document.getElementById('money-calculator').classList.add('visible');
          } else {}
        });
      }
    }, {
      ...
      key: "clickEventPanel",
      value: function clickEventPanel(el) {
        el.addEventListener('click', function () {
          var calcu = document.getElementById('money-calculator');

          if (calcu === null || calcu === undefined || calcu === false) {} else {
            calcu.classList.add('visible');
          }
        });
      }
    }, {
      ...
      key: "setAttrs",
      value: function setAttrs(e, a) {
        Object.entries(a).forEach(function (_ref3) {
      ...
      key: "calculateOffers",
      value: function calculateOffers() {
        var thisClass = this;
        var offers = document.querySelectorAll('.custom-package-content:not([money-calculator-expanded])');
        offers.forEach(function (offer) {
          var secondDiv = offer.querySelector('div > h3').parentElement; // secondDiv.setAttribute('style', 'width: 50% !important')

          secondDiv.classList.add('extension-offer-title-width');
          ...
          childSpans[0][1].textContent = currency + thisClass.calculate(price, 'yg');
          childSpans[1][0].textContent = 'Order Price';
          childSpans[1][1].textContent = currency + thisClass.calculate(price, 'op');
          childSpans[2][0].textContent = 'Buyer Pays';
          childSpans[2][1].textContent = currency + thisClass.calculate(price, 'bp');
      ...
      value: function calculate() {
        ...
        var b = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'op';
...
/***/ "./Fiverr-Seller-Assistant crome extension/assets/src/js/content/showBalance.js":
/*!**************************************************************************************!*\
  !*** ./Fiverr-Seller-Assistant crome extension/assets/src/js/content/showBalance.js ***!
  \**************************************************************************************/
/*! no static exports found */
                          ...
                          while (1) {
                            switch (_context.prev = _context.next) {
                              case 0:
                                ...
                                /**
                                 * First Css execute balance display: none; autometically
                                 * Then If Baance is not Hided, then It will add a class on Balance, to display
                                 */
                                  ...
                                  if (res.status === 200) {
                                    res.json().then(function (json) {
                                      ...
                                      if (json.status) {
                                        ...
                                        target.classList.add('d-block'); // target.setAttribute('style', 'display: none !important;')
                                      }
                                    });
                                  } else {
